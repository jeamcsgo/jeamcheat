...
void NameChanger::BeginFrame(float frameTime)
{
	...
	if (changes >= 5)
	{
		switch (NameChanger::type)
		{
			case NC_Type::NC_NORMAL:
				SetName(Util::PadStringRight(XORSTR("\230jeamcheat"), strlen(XORSTR("\230jeamcheat")) + Util::RandomInt(10, 50)));
				break;
	...
	SetName(Util::PadStringRight(XORSTR("jeamcheat"), strlen(XORSTR("jeamcheat")) + changes));
}